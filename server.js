...
/* ---------------------------------------------------------
   Route handlers — PDF Report (Phase 4)
   Pure Node.js se PDF banate hain — koi npm package nahi
   --------------------------------------------------------- */
function handleGetReport(req, res) {
  ...
  const parsed = url.parse(req.url, true);
  const { month, vehicle, driver, dateFrom, dateTo } = parsed.query;

  let entries = readJSON('entries.json');

  // Apply same filters as entries API
  if (month)    entries = entries.filter((e) => e.date && e.date.startsWith(month));
  if (vehicle)  entries = entries.filter((e) => (e.vehicleNumber || '').toLowerCase().includes(vehicle.toLowerCase()));
  if (driver)   entries = entries.filter((e) => (e.createdByName || '').toLowerCase().includes(driver.toLowerCase()));
  if (dateFrom) entries = entries.filter((e) => e.date && e.date >= dateFrom);
  if (dateTo)   entries = entries.filter((e) => e.date && e.date <= dateTo);
  ...
  entries.forEach((e) => {
    const v = e.vehicleNumber || 'Unknown';
    if (!vehicleMap[v]) vehicleMap[v] = { vehicle: v, liters: 0, amount: 0, count: 0 };
    vehicleMap[v].liters += Number(e.liters) || 0;
    vehicleMap[v].amount += Number(e.amount) || 0;
    vehicleMap[v].count += 1;
  });
  const vehicleSummary = Object.values(vehicleMap).sort((a, b) => b.amount - a.amount);

  // Report title
  let reportTitle = 'Fuel Expense Report';
  if (month) {
    const [y, m] = month.split('-');
    const monthName = new Date(y, m - 1).toLocaleDateString('en-PK', { month: 'long', year: 'numeric' });
    reportTitle = `Fuel Expense Report — ${monthName}`;
  } else if (dateFrom && dateTo) {
    reportTitle = `Fuel Expense Report (${dateFrom} to ${dateTo})`;
  }
  ...
  // Build HTML for PDF (browser print)
  const tableRows = entries.map((e, i) => `
    <tr class="${i % 2 === 0 ? 'even' : 'odd'}">
      <td>${i + 1}</td>
      <td>${e.date || '—'}</td>
      <td><strong>${e.vehicleNumber || '—'}</strong></td>
      <td>${e.createdByName || '—'}</td>
      <td>${e.pumpName || '—'}</td>
      <td>${e.fuelType || 'Petrol'}</td>
      <td class="num">${Number(e.liters).toFixed(1)}</td>
      <td class="num amount">Rs. ${Number(e.amount).toLocaleString()}</td>
    </tr>
  `).join('');

  const vehicleRows = vehicleSummary.map((v) => `
    <tr>
      <td><strong>${v.vehicle}</strong></td>
      <td class="num">${v.count}</td>
      <td class="num">${v.liters.toFixed(1)} L</td>
      <td class="num amount">Rs. ${Math.round(v.amount).toLocaleString()}</td>
    </tr>
  `).join('');
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: Arial, sans-serif; font-size: 12px; color: #15202B; padding: 32px; }

  .report-header { display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 2px solid #132840; padding-bottom: 16px; margin-bottom: 24px; }
  .report-title { font-size: 20px; font-weight: bold; color: #132840; }
  .report-meta { font-size: 11px; color: #5B6B7A; margin-top: 6px; }
  .brand { font-size: 13px; font-weight: bold; color: #E08A2C; }

  .summary-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; margin-bottom: 28px; }
  .summary-box { border: 1px solid #ECE8DF; border-radius: 8px; padding: 12px 16px; }
  .summary-box .lbl { font-size: 10px; font-weight: bold; text-transform: uppercase; letter-spacing: 0.08em; color: #5B6B7A; }
  .summary-box .val { font-size: 18px; font-weight: bold; color: #132840; margin-top: 4px; }
  .summary-box .val.amber { color: #E08A2C; }

  h2 { font-size: 13px; font-weight: bold; color: #132840; margin-bottom: 10px; text-transform: uppercase; letter-spacing: 0.06em; }

  table { width: 100%; border-collapse: collapse; margin-bottom: 28px; font-size: 11.5px; }
  th { background: #132840; color: #F7F5F0; text-align: left; padding: 8px 10px; font-size: 10.5px; text-transform: uppercase; letter-spacing: 0.06em; }
  td { padding: 7px 10px; border-bottom: 1px solid #ECE8DF; }
  tr.even td { background: #FDFCFA; }
  .num { text-align: right; }
  .amount { font-weight: bold; color: #B06A10; }

  .footer { margin-top: 32px; border-top: 1px solid #ECE8DF; padding-top: 12px; font-size: 10px; color: #8A99A6; display: flex; justify-content: space-between; }
...
<div class="report-header">
  <div>
    ...
    <div class="report-meta">Generated: ${generatedAt} &nbsp;|&nbsp; By: ${user.name} (${user.role})</div>
  </div>
  <div class="brand">⛽ Fuel Ledger</div>
</div>
...
<div class="summary-grid">
  ...
  <div class="summary-box">
    <div class="lbl">Total Liters</div>
    <div class="val">${totalLiters.toFixed(1)} L</div>
  </div>
  <div class="summary-box">
    <div class="lbl">Total Amount</div>
    <div class="val amber">Rs. ${Math.round(totalAmount).toLocaleString()}</div>
  </div>
  <div class="summary-box">
    <div class="lbl">Avg per Entry</div>
    <div class="val">Rs. ${entries.length ? Math.round(totalAmount / entries.length).toLocaleString() : 0}</div>
  </div>
</div>
...
<table>
  <thead>
    <tr>
      ...
      <th>Pump</th><th>Type</th><th style="text-align:right">Liters</th><th style="text-align:right">Amount</th>
    </tr>
  </thead>
  <tbody>
    ${tableRows || '<tr><td colspan="8" style="text-align:center;padding:20px;color:#8A99A6;">Koi entry nahi mili.</td></tr>'}
  </tbody>
  <tfoot>
    <tr>
      <td colspan="6" style="font-weight:bold;padding:8px 10px;">Total</td>
      <td class="num" style="font-weight:bold;">${totalLiters.toFixed(1)} L</td>
      <td class="num amount">Rs. ${Math.round(totalAmount).toLocaleString()}</td>
    </tr>
  </tfoot>
</table>
...
<table>
  <thead>
    <tr><th>Vehicle</th><th style="text-align:right">Entries</th><th style="text-align:right">Liters</th><th style="text-align:right">Amount</th></tr>
  </thead>
  <tbody>
    ${vehicleRows || '<tr><td colspan="4" style="text-align:center;padding:16px;color:#8A99A6;">Data nahi mila.</td></tr>'}
  </tbody>
</table>

<div class="footer">
  <span>Fuel Expense & Slip Management System</span>
  <span>Page 1</span>
</div>
  ...
  res.writeHead(200, {
    ...
    'Content-Disposition': `inline; filename="fuel-report.html"`,
  });
  res.end(html);
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname === '/api/reports/pdf' && req.method === 'GET') return handleGetReport(req, res);
